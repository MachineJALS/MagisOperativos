// server/nodes/conversionNode1.js - NODO DE CONVERSIÓN #1
...
async function updateNodeStats() {
    try {
        // Simular métricas del sistema (en producción usarías os.cpus(), etc.)
        nodeState.cpuUsage = Math.min(100, Math.max(10, Math.random() * 60 + nodeState.activeTasks * 15));
        nodeState.memoryUsage = Math.min(100, Math.max(20, Math.random() * 40 + nodeState.activeTasks * 10));
...
async function processTask(taskId, taskType, taskData) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Simular conversión (en producción usarías FFmpeg real)
            const processingTime = Math.random() * 30000 + 15000; // 15-45 segundos
...
app.get('/api/tasks/:taskId/status', (req, res) => {
    ...
    const progress = Math.min(95, (elapsed / 45000) * 100); // Máximo 95% hasta completar
...
async function initializeNode() {
    ...
    if (registered) {
        ...
        setInterval(updateNodeStats, 10000); // Cada 10 segundos
...
app.listen(3002, () => {
    console.log(`🎯 Nodo de conversión ${NODE_CONFIG.id} ejecutándose en puerto 3002`);
    initializeNode();
});
